 ...
 * @param columns : кол-во столбцов мт
 * @param result : сама матрица (указатель)
 * @return int 1 - Ошибка, некорректная матрица
 *             0 - OK
 */
    ...
    if ((A->columns == B->columns && A->rows == B->rows)
        ...
        for (int i = 0; i < A->rows; i++)
            for (int a = 0; a < A->columns; a++)
                if (fabs(A->matrix[i][a] - B->matrix[i][a]) > EPS)
                    error = 0;
    } else {
        error = 0;
    }
...
int s21_mult_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
    ...
    (B->columns == A->rows || B->rows == A->columns) ? 0 : 2 : 1;
    if (err == 0) {
...
int s21_transpose(matrix_t *A, matrix_t *result) {
    ...
    if (s21_check_matrix(A)) {
        ...
        for (int i = 0; i < result->rows; i++)
                for (int a = 0; a < result->columns; a++)
                   result->matrix[i][a] = A->matrix[a][i];
    } else {
        error = ERROR;
    }
    return error;
...
int s21_inverse_matrix(matrix_t *A, matrix_t *result) {
    ...
    if (!s21_check_matrix(A)) {error = ERROR;
        ...
        if ( A->rows != A->columns) {error = ERROR_CAL;
        } else {
